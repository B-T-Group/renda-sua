...
function objective(order: OrderLike, ctx: OrderViewModelContext): string {
...
function urgencyFor(
  ...
  if (order.current_status === 'assigned_to_agent') {
    const pickupDeadline = order.pickup_due_at || order.pickup_by;
    if (pickupDeadline) {
      return {
        deadlineAt: pickupDeadline,
        label: ctx.t('orders.delivery.urgency.pickupBy', 'Pick up by'),
        overdue: isOverdue(pickupDeadline, ctx.now),
      };
    }
  }
  if (
...
function deliveryActions(
  ...
  if (s === 'assigned_to_agent') {
    if (!(order.pickup_extension_minutes && order.pickup_extension_minutes > 0)) {
      actions.push({
        id: 'pickup_delay',
        labelKey: 'orders.delivery.actions.runningLate',
        labelDefault: 'Running late',
        variant: 'outlined',
      });
    }
    actions.push({
      id: 'report_pickup_issue',
      labelKey: 'orders.delivery.actions.reportPickupIssue',
      labelDefault: 'Report a problem',
      variant: 'outlined',
    });
    actions.push({
      id: 'drop_order',
...
function weightLabel(order: OrderLike): string | null {
  ...
  const total = items.reduce(
    (sum, i) => sum + (i.item?.weight ?? 0) * (i.quantity ?? 1),
    0
  );
  if (!total) return null;
  const unit =
    items.find((i) => i.item?.weight_unit)?.item?.weight_unit ?? 'kg';
  return `${total} ${unit}`;
}
