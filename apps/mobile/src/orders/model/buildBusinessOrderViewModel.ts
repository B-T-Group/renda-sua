...
function requiredActionLabel(
  ...
  if (primaryId === 'none' && order.current_status === 'ready_for_pickup') {
    return ctx.t('orders.business.hero.waitingAgent', 'Waiting for Agent');
  }
  if (
    primaryId === 'none' &&
    ['delivered', 'complete'].includes(order.current_status)
  ) {
    return ctx.t('orders.business.hero.completed', 'Order Completed');
  }
  // Avoid duplicating statusMessage when there is no discrete required action.
  return '';
}
