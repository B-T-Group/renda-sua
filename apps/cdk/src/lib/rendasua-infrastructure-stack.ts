...
export class RendasuaInfrastructureStack extends cdk.Stack {
  constructor(
    ...
    // FIFO SQS + Lambda for sale item AI auto-review
    const itemAiReviewQueue = new sqs.Queue(
      this,
      `ItemAiReviewQueue-${environment}`,
      {
        queueName: `item-ai-review-${environment}.fifo`,
        fifo: true,
        contentBasedDeduplication: true,
        retentionPeriod: cdk.Duration.days(14),
        // Must exceed Lambda timeout (propose + image cleanup can be long).
    ...
    const itemAiReviewHandler = new lambda.Function(
      this,
      `ItemAiReviewHandler-${environment}`,
      {
        functionName: `item-ai-review-handler-${environment}`,
        runtime: lambda.Runtime.PYTHON_3_11,
        handler: 'handler.handler',
    ...
    itemAiReviewHandler.addEventSource(
    ...
    new cdk.CfnOutput(this, `ItemAiReviewQueueUrl-${environment}`, {
      value: itemAiReviewQueue.queueUrl,
      description: 'SQS Queue URL for sale item AI review',
