...
/** Last-resort aliases when country-state-city does not recognize a geocoder label. */
const COMMON_GEOCODE_COUNTRY_NAMES: Record<string, string> = {
  'united states': 'US',
  usa: 'US',
...
export function lookupCommonGeocodeCountry(
  countryName: string | null | undefined
): string {
  if (!countryName?.trim()) return '';
  return COMMON_GEOCODE_COUNTRY_NAMES[countryName.toLowerCase().trim()] ?? '';
}
...
export async function findCountryCodeFromGeocodeName(
  ...
  return lookupCommonGeocodeCountry(countryName);
}
