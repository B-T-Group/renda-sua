...
| `price` | `catalog_price`, `list_price` | Catalog `items.price` (> 0 unless `export_available`) |
| `selling_price` | `sale_price` | Inventory selling price; default = `price` |
| `export_available` | `pricing_not_applicable`, `sur_demande`, `quote_only` | Boolean; when true, set `items.export_available` and allow missing/zero shopper price |
...
INSERT INTO public.items (
  ...
  coalesce($export_available, false)
)
RETURNING id;
```
