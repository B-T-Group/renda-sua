 ...
 * Keep in sync with apps/frontend/src/utils/rentalPhase.ts
 */
...
export interface RentalPhaseInfo {
  ...
  /** Business action-queue bucket when this row needs ops attention. */
...
function hubGroupFor(phase: RentalPhase, role: RentalPhaseRole): RentalHubGroup {
  ...
  // business hub grouping for request lists
  if (
    phase === 'requested' ||
    phase === 'reserved' ||
    phase === 'ready_for_pickup' ||
...
/**
 * Resolve user-facing phase from request and/or booking status.
 * Booking status wins when present (except cancelled/unavailable terminal request states).
 */
export function resolveRentalPhase(
  ...
  if (
    ...
    book === 'proposed' ||
    req === 'available' ||
    (req === 'booked' && book === 'proposed')
  ) {
    phase = 'offer_ready';
  } else if (req === 'pending' || (!book && !req)) {
    phase = req === 'pending' || !book ? (req === 'pending' ? 'requested' : 'done') : 'done';
  ...
  // Refine: pending request with no booking
  if (req === 'pending' && !book) {
    phase = 'requested';
  }
...
/** Resolve phase from a booking status alone (booking detail screens). */
...
/** @deprecated Prefer resolveBookingPhase / resolveRentalPhase nextStepKey */
