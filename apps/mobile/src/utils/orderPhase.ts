...
function isShipping(input: OrderPhaseInput): boolean {
  return input.fulfillmentMethod === 'shipping';
}
...
function isPickupPinReady(input: OrderPhaseInput): boolean {
  if (!isPickup(input) || !isPinEligible(input)) return false;
...
function resolvePhase(input: OrderPhaseInput): OrderPhase {
  ...
  if (s === 'confirmed' || s === 'preparing' || s === 'awaiting_shipment') {
    return 'prepare';
  }
  if (s === 'ready_for_pickup') return 'ready';
  if (s === 'shipped' || s === 'in_delivery') return 'in_delivery';
  if (
    s === 'assigned_to_agent' ||
    s === 'picked_up' ||
    s === 'in_transit' ||
...
function nextStepKeyFor(
  ...
  const shipping = isShipping(input);
  ...
  if (phase === 'done') {
    ...
    if (s === 'cancelled' || s === 'failed') return null;
    return null;
  }
  ...
  if (phase === 'prepare') {
    if (shipping) {
      return role === 'client'
        ...
        : role === 'business'
          ? 'orders.nextStep.prepareShippingBusiness'
          : null;
    }
    return role === 'client'
  ...
  if (phase === 'in_delivery') {
    if (shipping) {
      return role === 'client'
        ? 'orders.nextStep.shippingClient'
        : role === 'business'
          ? 'orders.nextStep.shippingBusiness'
          : null;
    }
...
function hubGroupFor(
  ...
  if (role === 'client') {
    ...
    if (
      isShipping(input) &&
      (input.status === 'shipped' || input.status === 'in_delivery')
    ) {
      return 'action_needed';
    }
    if (
  ...
  // agent
  if (isShipping(input)) return 'waiting';
...
function primaryActionFor(
  ...
  if (isShipping(input)) return shippingPrimaryAction(phase, role, input);
  return standardPrimaryAction(phase, role, input);
}

function shippingPrimaryAction(
  phase: OrderPhase,
  role: OrderPhaseRole,
  input: OrderPhaseInput
): OrderPrimaryActionId {
  ...
  if (role === 'client') {
    ...
    if (s === 'shipped' || s === 'in_delivery') return 'confirm_receipt';
    if (s === 'complete') return 'rate';
    return 'none';
  }
  if (s === 'refund_requested') return 'open_refunds';
  if (phase === 'confirm') return 'confirm';
  if (phase === 'prepare') return 'mark_shipped';
  if (phase === 'in_delivery') return 'update_tracking';
  return 'none';
}

function standardPrimaryAction(
  phase: OrderPhase,
  role: OrderPhaseRole,
  input: OrderPhaseInput
): OrderPrimaryActionId {
  ...
  if (input.reconciliationStatus === 'pending_manual_reconciliation' && role === 'business') {
...
/** Progress steps for UI steppers — pickup skips delivery transit. */
