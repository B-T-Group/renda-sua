...
export function mapExhaustedHasuraQueryError(error: unknown): never {
  ...
  if (isTransientHasuraNetworkError(error)) {
    throw new HttpException(hasuraUnavailableResponse(), HttpStatus.SERVICE_UNAVAILABLE, {
...
function matchesTransientMessage(err: HasuraErrorShape): boolean {
  const text = `${err.message || ''} ${stringifyResponseError(err.response?.error)}`;
...
async function retryHasuraAttempt<T>(
  ...
  try {
    ...
    return retryHasuraAttempt(request, logger, maxAttempts, delayMs, attempt + 1);
  }
}
