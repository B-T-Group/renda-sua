...
def send_aggregated_proximity_notification(
    ...
    """
    Send aggregated proximity notification to agent via SendGrid.
    
    Args:
        agent_location: Agent location with user info
        order_count: Number of nearby orders
    ...
    """
    log_info(
        "Preparing to send aggregated proximity notification",
        agent_id=agent_location.agent_id,
    ...
    try:
        ...
        if proximity_radius_km < 1:
            proximity_radius_str = f"{proximity_radius_km * 1000:.0f} m"
        else:
            proximity_radius_str = f"{proximity_radius_km:.0f} km"
        ...
        dynamic_template_data = {
            "agent_name": agent_name,
            "order_count": order_count,
            "proximity_radius": proximity_radius_str,
            "currentYear": current_year,
        }
        ...
        # Send email
        log_info(
            "Sending aggregated email via SendGrid",
            to=agent_email,
            template_id=template_id[:10] + "...",
            order_count=order_count,
            api_key_preview=sendgrid_api_key[:8] + "...",
        ...
        try:
            ...
            if response.status_code >= 200 and response.status_code < 300:
                log_info(
                    "Aggregated notification sent successfully",
                    agent_email=agent_email,
                    order_count=order_count,
                    status_code=response.status_code,
                )
        ...
        log_error(
            "Error sending aggregated SendGrid notification",
            error=e,
            agent_email=agent_email if 'agent_email' in locals() else "unknown",
            order_count=order_count,
        )
        import traceback
        traceback.print_exc()
...
def send_aggregated_notifications_to_agents(
    ...
    """
    Send aggregated notifications to agents (one per agent).
    
    Args:
        agent_locations: List of agent locations to notify
        agent_order_counts: Dictionary mapping agent_id to order count
        proximity_radius_km: Proximity radius in kilometers
    ...
    """
    log_info(
        "Starting aggregated notification sending",
    ...
    for idx, agent_location in enumerate(agent_locations, 1):
        agent_id = agent_location.agent_id
        order_count = agent_order_counts.get(agent_id, 0)
        
        if order_count == 0:
            log_info(
                f"Skipping agent {idx} of {len(agent_locations)} - no nearby orders",
                agent_id=agent_id,
            )
            continue
        
        log_info(
            f"Sending aggregated notification {idx} of {len(agent_locations)}",
            agent_id=agent_id,
            order_count=order_count,
        )
        
        success = send_aggregated_proximity_notification(
            agent_location,
            order_count,
            proximity_radius_km,
            sendgrid_api_key,
    ...
    log_info(
        "Aggregated notification sending completed",
        success_count=success_count,
        total_count=len(agent_locations),
    )
    
    print(f"Sent {success_count} out of {len(agent_locations)} aggregated notifications")
