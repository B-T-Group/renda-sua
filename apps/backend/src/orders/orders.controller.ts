...
export class OrdersController {
  ...
  @ApiOperation({ summary: 'Get agent earnings for a specific order' })
  @ApiResponse({
    status: 200,
    description: 'Agent earnings calculated successfully',
    schema: {
      type: 'object',
      properties: {
        success: { type: 'boolean', example: true },
        earnings: {
          type: 'object',
          properties: {
            totalEarnings: { type: 'number', example: 1200 },
            baseDeliveryCommission: { type: 'number', example: 500 },
            perKmDeliveryCommission: { type: 'number', example: 700 },
            currency: { type: 'string', example: 'USD' },
          },
        },
        message: {
          type: 'string',
          example: 'Agent earnings calculated successfully',
        },
      },
    },
  })
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        success: { type: 'boolean', example: false },
        error: { type: 'string', example: 'User is not an agent' },
      },
    },
  })
  @ApiResponse({
  ...
  async getAgentEarnings(
    ...
    @CurrentUser() _auth0User: any
  ) {
    try {
      // Get user profile to check if they are an agent
      ...
      const agentResponse = await this.hasuraUserService.executeQuery(
        agentQuery,
        {
          userId: user.id,
        }
      ...
      if (!agent) {
        throw new HttpException('User is not an agent', HttpStatus.FORBIDDEN);
      }
      ...
      if (errorMessage.includes('not found')) {
        statusCode = HttpStatus.NOT_FOUND;
      } else if (errorMessage.includes('not an agent')) {
        statusCode = HttpStatus.FORBIDDEN;
      }
